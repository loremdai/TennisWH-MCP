...
def search_tennis_products(
    ...
    print(
        f"🔍 Searching products: query='{query}', category={category}", file=sys.stderr
    )
    ...
    should_use_smart_search = (
        len(products) >= 8  # Many results
        and not any("error" in str(p) for p in products)  # No errors
        and (
            len(insights.get("brands", [])) >= 3 or len(insights.get("types", [])) >= 3
        )  # Multiple filter options
    )

    if should_use_smart_search:
        print(
            f"🧠 Using smart search - found {len(products)} products with filtering options",
            file=sys.stderr,
        )
        ...
        for product in products:
            if product.get("product_url") and "source_citation" not in product:
                product["source_citation"] = (
                    f"[Tennis Warehouse]({product['product_url']})"
                )
            ...
            if product.get("source_citation") and not product.get("name", "").endswith(
                ...
                product["name"] = (
                    f"{product.get('name', 'Unknown Product')} {product['source_citation']}"
                )

        print(
            f"✅ Found {len(products)} products (using standard search)",
            file=sys.stderr,
        )
        return products
...
def get_product_specs(product_url: str) -> Dict[str, Any]:
    ...
    - User asks: "Compare the Stiffness of these two racquets"
      → Search for each racquet, then call this tool for each URL
    ...
    print(f"📊 Getting product specifications from URL", file=sys.stderr)
    ...
    if "error" in result:
        print(f"❌ Failed to get specs: {result['error']}", file=sys.stderr)
        return result

    specs = result.get("specs", {})
    print(f"✅ Retrieved {len(specs)} specifications", file=sys.stderr)
...
def search_tennis_bags(
    ...
    # Criteria for smart search: many results + multiple filter options
    should_use_smart_search = (
        len(filtered_bags) >= 6  # Many bags found
        and (
            len(insights.get("brands", [])) >= 3 or len(insights.get("types", [])) >= 3
        )  # Multiple filter options
    )
    ...
    if should_use_smart_search:
        print(
            f"🧠 Using smart search for bags - found {len(filtered_bags)} bags with filtering options",
            file=sys.stderr,
        )
        ...
        print(
            f"✅ Found {len(filtered_bags)} matching bags (using standard search)",
            file=sys.stderr,
        )
...
def search_tennis_racquets(
    ...
    print(
        f"🎾 Searching racquets: brand={brand}, weight={weight_range}", file=sys.stderr
    )
    ...
    # Criteria for smart search: many results + multiple filter options
    should_use_smart_search = (
        len(products) >= 6  # Many racquets found
        and not any("error" in str(p) for p in products)  # No errors
        and (
            len(insights.get("brands", [])) >= 3 or len(insights.get("types", [])) >= 3
        )  # Multiple filter options
    ...
    if should_use_smart_search:
        print(
            f"🧠 Using smart search for racquets - found {len(products)} racquets with filtering options",
            file=sys.stderr,
        )
        ...
        print(
            f"✅ Found {len(products)} racquets (using standard search)",
            file=sys.stderr,
        )
        return products
...
def search_tennis_shoes(
    ...
    print(
        f"👟 Searching shoes: gender={gender}, brand={brand}, court={court_type}",
        file=sys.stderr,
    )
    ...
    # Criteria for smart search: many results + multiple filter options
    should_use_smart_search = (
        len(products) >= 6  # Many shoes found
        and not any("error" in str(p) for p in products)  # No errors
        and (
            len(insights.get("brands", [])) >= 3 or len(insights.get("types", [])) >= 3
        )  # Multiple filter options
    ...
    if should_use_smart_search:
        print(
            f"🧠 Using smart search for shoes - found {len(products)} shoes with filtering options",
            file=sys.stderr,
        )
        ...
        print(
            f"✅ Found {len(products)} shoes (using standard search)", file=sys.stderr
        )
        return products
...
def check_product_availability(product_name: str) -> Dict[str, Any]:
    ...
    print(
        f"✅ Product found: {result['name']} - {result['availability']}",
        file=sys.stderr,
    )
    return result
...
@mcp.tool()
def smart_search_tennis(query: str, max_results: int = 20) -> List[Dict[str, Any]]:
    ...
    NOTE: This tool does NOT return detailed technical specifications like:
    - Swingweight, Stiffness, Balance, Head Size, etc.

    If user asks about technical parameters, you MUST:
    1. Use this tool to find the product and get its URL
    2. Then call get_product_specs with the product URL to get detailed specs

    Example workflow:
    - User: "What's the Swingweight of Wilson Blade 98?"
    - Step 1: Call smart_search_tennis(query="Wilson Blade 98")
    - Step 2: Extract product_url from results
    - Step 3: Call get_product_specs(product_url=url) to get Swingweight
    ...
    print(
        f"✅ Smart search complete: {insights.get('total_products', 0)} products found",
        file=sys.stderr,
    )
    return result
...
def handle_search_option(
    query: str, option_number: int, max_results: int = 20
) -> List[Dict[str, Any]]:
    ...
    if option_number == 1 and len(brands) > 0:
        ...
        for i, brand in enumerate(brands[:10], 1):  # Show top 10 brands
            brand_options.append(
                f"   {i}. {brand.get('display_name', brand.get('name', 'Unknown'))}"
            )
        ...
        for i, type_item in enumerate(types[:10], 1):  # Show top 10 types
            type_options.append(
                f"   {i}. {type_item.get('display_name', type_item.get('name', 'Unknown'))}"
            )
        ...
        if products and not any("error" in p for p in products):
            ...
            response = (
                f"📋 All {len(products)} results for '{query}' from Tennis Warehouse:"
            )
            return [
                {
                    "message": response,
                    "source_info": "Click source links to view full details and purchase on Tennis Warehouse",
                }
            ] + products
        else:
        ...
        return [
            {
                "error": f"Invalid option {option_number}. Please choose 1 (brands), 2 (types), 3 (summary), or 4 (list all)."
            }
        ]
